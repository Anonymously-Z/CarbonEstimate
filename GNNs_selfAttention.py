...
class GNN(torch.nn.Module):
    def __init__(self, in_feats, h_feats, num_class):
        ...
        self.local_attention = nn.MultiheadAttention(embed_dim=h_feats, num_heads=1)
    ...
    def forward(self, graph, feat, eweight=None):
        ...
        # Attention mechanism (modify this part)
        h_att = h.unsqueeze(0)  # Add batch dimension for attention
        h_att, _ = self.local_attention(h_att, h_att, h_att)
        h_att= h_att.squeeze(0)  # Remove batch dimension
        # Add attention output to the input of the second GCN layer
        h = h + h_att
        # h = F.relu(h)
        # 批归一化
        # h = self.batch_norm(h)
